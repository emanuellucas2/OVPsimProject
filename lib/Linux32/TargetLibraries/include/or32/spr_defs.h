...
/* C macros to read/write special-purpose registers */
#define MTSPR(_SPR, _VALUE)                                             \
  ({unsigned long value = _VALUE;                                       \
    asm volatile ("l.mtspr  %0,%1,0": :"r" (_SPR), "r" (value));        \
  })

#define MFSPR(_SPR)                                                     \
  ({unsigned long value;                                                \
    asm volatile ("l.mfspr  %0,%1,0" : "=r" (value) : "r" (_SPR));      \
    value;                                                              \
  })
