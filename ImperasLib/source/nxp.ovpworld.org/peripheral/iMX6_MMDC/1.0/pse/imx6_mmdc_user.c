...
#define MMDC_MAPSR bport1_ab_data.MMDC_MAPSR
...
PPM_REG_WRITE_CB(WriteMMDC_MAPSR) {
    Uns32 old = *(Uns32*)user;
    *(Uns32*)user = (*(Uns32*)user & 0xffccf78e) | (data & 0x330871);
    if(!MMDC_MAPSR.bits.PSD) {
        // power saving has been enabled
        MMDC_MAPSR.bits.PSS = 1;
        if ((old & BPORT1_AB_MMDC_MAPSR_PSD) && PSE_DIAG_HIGH) {
            bhmMessage("I", PREFIX, "Power Savign Enabled");
        }
    } else {
        MMDC_MAPSR.bits.PSS = 0;
    }
}
