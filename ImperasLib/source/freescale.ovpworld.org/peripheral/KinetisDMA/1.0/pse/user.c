...
PPM_NBYTE_WRITE_CB(regWriteTCD) {
    ...
    {
        case 0x0: // SADDR
            tcdReg[chNum].TCD_SADDR.value = (tmpd & 0xffffffff);
            if (diagnosticLevel >= 3)
                bhmMessage("I", "KinetisDMA", "Write SADDR Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
            break;
        case 0x8: // NBYTES
            tcdReg[chNum].TCD_NBYTES.value = (tmpd & 0xffffffff);
            if (diagnosticLevel >= 3)
                bhmMessage("I", "KinetisDMA", "Write NBYTES Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
            break;
        case 0xc: // SLAST
            tcdReg[chNum].TCD_SLAST.value = (tmpd & 0xffffffff);
            if (diagnosticLevel >= 3)
                bhmMessage("I", "KinetisDMA", "Write SLAST Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
            break;
        case 0x10: // DADDR
            tcdReg[chNum].TCD_DADDR.value = (tmpd & 0xffffffff);
            if (diagnosticLevel >= 3)
                bhmMessage("I", "KinetisDMA", "Write DADDR Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
            break;
        case 0x18: // DLASTSGA
            tcdReg[chNum].TCD_DLASTSGA.value = (tmpd & 0xffffffff);
            if (diagnosticLevel >= 3)
                bhmMessage("I", "KinetisDMA", "Write DLASTSGA Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
            break;
        default:
            bhmMessage("W", periphName, "Unhandled address in TCD block");
            break;  
...
PPM_NBYTE_WRITE_CB(regWriteTCD16) {
    ...
    Uns16 tmpd = *(Uns16*)data;
    if (bytes == 2) {
        switch (baseOffset)
        {
            case 0x4:
                tcdReg[chNum].TCD_SOFF.value = (tmpd);
                if (diagnosticLevel >= 3)
                    bhmMessage("I", "KinetisDMA", "Write SOFF Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                break;
            case 0x6:
                tcdReg[chNum].TCD_ATTR.value = (tmpd);
                if (diagnosticLevel >= 3)
                    bhmMessage("I", "KinetisDMA", "Write ATTR Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                break;
            case 0x14:
                tcdReg[chNum].TCD_DOFF.value = (tmpd);
                if (diagnosticLevel >= 3)
                    bhmMessage("I", "KinetisDMA", "Write DOFF Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                break;
            case 0x16:
                if (tmpd & 0x8000)
                    tcdReg[chNum].TCD_CITER.value = (tmpd);
                else
                    tcdReg[chNum].TCD_CITER.value = (tmpd);
                if (diagnosticLevel >= 3)    
                    bhmMessage("I", "KinetisDMA", "Write CITER Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                break;
            case 0x1c:
                tcdReg[chNum].TCD_CSR.value = (tmpd);
                if (diagnosticLevel >= 3)
                    bhmMessage("I", "KinetisDMA", "Write CSR Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                checkCSRReg(chNum);
                break;
            case 0x1e:
                if (tmpd & 0x8000)
                    tcdReg[chNum].TCD_BITER.value = (tmpd);
                else
                    tcdReg[chNum].TCD_BITER.value = (tmpd);
                if (diagnosticLevel >= 3)
                    bhmMessage("I", "KinetisDMA", "Write BITER Ch = %d  size = %d val = 0x%x", chNum, bytes, tmpd);
                break;
        } 
    } else {
