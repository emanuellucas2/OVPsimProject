...
class riscv_Ibex_RV32IMCB : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "Ibex_RV32IMCB");
        return p;
    }
    ...
    const char *getModel() {
        return opVLNVString (NULL, "lowrisc.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, OP_VLNV_FATAL);
    }
  ...
  public:
    ...
    riscv_Ibex_RV32IMCB(tlmModule &parent, sc_module_name name)
    ...
    riscv_Ibex_RV32IMCB(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34)
