...
# Platform
#
ihwnew -name ArmIntegratorCP  -vendor arm.ovpworld.org -library platform -version 1.0 \
       -stoponctrlc \
...
ihwaddnet -instancename ir23
ihwaddnet -instancename ir24
...
# Processor
#
ihwaddprocessor -instancename arm1 \
                -type arm \
...
ihwsetparameter -handle arm1 -name variant            -value ARM926EJ-S   -type enum
ihwsetparameter -handle arm1 -name compatibility      -value ISA          -type enum
ihwsetparameter -handle arm1 -name showHiddenRegs     -value 0            -type bool

#
# Connect processor to bus
#
ihwconnect -bus bus1 -instancename arm1 -busmasterport INSTRUCTION
ihwconnect -bus bus1 -instancename arm1 -busmasterport DATA

# processor to interrupt
ihwconnect -net irq -instancename arm1 -netport irq
ihwconnect -net fiq -instancename arm1 -netport fiq
...
ihwaddclp -allargs -usagemessage "Basic Usage. Linux ArmIntegratorCP --kernel <kernel image> --ramdisk <initial ramdisk>. BareMetal ArmIntegratorCP --program <elf file> --nographics"
ihwaddclparg -name kernel       -type stringvar -group "user platform config Linux" -description "The Linux Kernel image e.g. zImage"
ihwaddclparg -name ramdisk      -type stringvar -group "user platform config Linux" -description "Boot Linux Kernel from the specified ramdisk image e.g. fs.img"
ihwaddclparg -name uartport     -type uns64var  -group "user platform config" -description "set the port number to open on the UART (uart1)"
ihwaddclparg -name uartconsole  -type boolvar   -group "user platform config" -description "open a console terminal on the UART (uart1)"
ihwaddclparg -name semihost     -type stringvar -group "user platform config" -description "Specify a semihost library (full path)"
