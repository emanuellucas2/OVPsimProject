...
BlockDriverStateP bdrv_open(Uns8 drive, const char *filename, Int32 flags, Uns32 diag)
{
    ...
    //
    // This needs addressing correctly
    //
#if (IMPERAS_WINDOWS==1)
#define tosStatStruct _stati64
#define tosFStat _fstati64
#else
#define tosStatStruct stat
#define tosFStat fstat
#endif

    if ((bs->fd = open(filename, rw)) >= 0) {
        struct tosStatStruct st;
        if (tosFStat(bs->fd, &st) >= 0) {
            bs->present      = True;
            bs->totalSectors = st.st_size / SECT_SIZE;
            bs->cylinders    = bs->totalSectors / (bs->heads * bs->sectors);
            if(DIAG_HIGH_DEBUG) {
                vmiMessage("I", DRIVE_PREFIX, "open drive '%s' result:%d (size " FMT_64d ") sectors=" FMT_64d " cylinders=%d",
                        filename, bs->fd, (Uns64)st.st_size, bs->totalSectors, bs->cylinders);
            }
            return bs;
        } else {
            bdrv_close(bs);
...
Int32 bdrv_write(BlockDriverStateP bs, Uns64 sector_num, const Uns8 *buf, Uns32 nb_sectors, Uns32 diag)
{
    if(bs->delta) {
        ...
        if(r != 0) {
            vmiMessage("E", DRIVE_PREFIX, "Failed to write to disk image (errno=%u)", r);
            return r;
        }
    }
    bs->totalWrites++;
...
{
    vmiMessage("I", DRIVE_PREFIX, "drive %d ejected!", bs->number);
}

void bdrv_set_change_cb(BlockDriverStateP bs, void (*change_cb)(void *opaque), void *opaque)
{
    vmiMessage("W", DRIVE_PREFIX, "bdrv_set_change_cb unsupported");
}

void bdrvStats(void)
{
    Uns32 i;
    vmiMessage("I", DRIVE_PREFIX, "Drive statistics:");
