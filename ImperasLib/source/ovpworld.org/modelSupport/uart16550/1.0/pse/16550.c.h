...
//
static inline Uns32 data_bits   (void) { return (bport1_ab_data.lcr.bits.dls + 5);      }
...
static inline Uns32 stop_bits   (void) { return (bport1_ab_data.lcr.bits.stop ? 2 : 1); }
...
static void startRxThread() {
    stopRxThread();
    rxThreadHandle = bhmCreateThread(channelReadThread, NULL, "channelReadThread", &threadStack[TOP]);
}
...
static Uns32 getFifoSize() {
    if (bport1_ab_data.fcr.bits.fifoEn) {
...
static inline Bool rxTrigger() {
    ...
    if (bport1_ab_data.fcr.bits.fifoEn && (state.rxTrigLvl != 0)) {
        // When in fifo mode, indicate if trigger level has been reached
...
static intStateE getInterruptState() {
    ...
    if (bport1_ab_data.ier_dlh.bits.elsi && state.overrunErr) {
        intState |= INT_RX_LINE_STATUS;
    }
    if (bport1_ab_data.ier_dlh.bits.etbei && state.threInterrupt) {
    ...
    //if (bport1_ab_data.ier_dlh.bits.erbfi && rxTrigger()) {
    if (bport1_ab_data.ier_dlh.bits.erbfi && !Rx.empty) {
...
PPM_REG_WRITE_CB(writeIER_DLH) {
    ...
    if (bport1_ab_data.lcr.bits.dlab) {
        ...
        Bool etbeiOld = bport1_ab_data.ier_dlh.bits.etbei;

        bport1_ab_data.ier_dlh.value = data & 0x0f;

        if (!etbeiOld && bport1_ab_data.ier_dlh.bits.etbei) {
...
static Uns8 getIerDlh() {
    ...
    if (bport1_ab_data.lcr.bits.dlab) {
        ...
        return bport1_ab_data.ier_dlh.value;
...
static Uns8 getIir() {
    ...
    if (intState & INT_RX_LINE_STATUS) {
        bport1_ab_data.iir.bits.intid = INT_ID_RX_LINE_STATUS;
    } else if (intState & INT_RX_DATA_AVAIL) {
        bport1_ab_data.iir.bits.intid = INT_ID_RX_DATA_AVAIL;
    } else if (intState & INT_TX_EMPTY) {
        bport1_ab_data.iir.bits.intid = INT_TD_TX_EMPTY;
    } else {
        bport1_ab_data.iir.bits.intid = INT_ID_NO_INT;
    }

    // both bits of fifoen field set when fifos are enabled
    if (bport1_ab_data.fcr.bits.fifoEn) {
        bport1_ab_data.iir.bits.fifoen |= 0x3;
    }
...
PPM_REG_WRITE_CB(writeFCR) {
    ...
    if (state.noFifo) {
        ...
        Bool oldFifoEn = bport1_ab_data.fcr.bits.fifoEn;

        bport1_ab_data.fcr.value = (data & 0xcf);

        Bool newFifoEn     = bport1_ab_data.fcr.bits.fifoEn;
        Bool enableChanged = (oldFifoEn != newFifoEn);

        // Reset fifo when enable changes or corresponding reset bit is set
        if (enableChanged || bport1_ab_data.fcr.bits.fifoRcvrRst) {
            bport1_ab_data.fcr.bits.fifoRcvrRst = 0;    // self clearing bit
            resetFifo(&Rx);
        ...
        if (enableChanged || bport1_ab_data.fcr.bits.fifoXmitRst) {
            bport1_ab_data.fcr.bits.fifoXmitRst = 0;    // self clearing bit
            resetFifo(&Tx);
        }
        ...
        if (newFifoEn) {
            Uns32 size = getFifoSize();
            switch (bport1_ab_data.fcr.bits.fifoRcvrTrig) {
            ...
            switch (bport1_ab_data.fcr.bits.fifoXmitTrig) {
...
static Uns8 getLsr() {
    ...
    bport1_ab_data.lsr.bits.dr   = !Rx.empty;
