...
static OR1K_MORPH_FN(morphSYS) {
    ...
    // correct next instruction address in instruction attributes
    vmimtUncondJump(0, SYS_ADDRESS, VMI_NOREG, vmi_JH_CALLINT);
}
...
//
// Are FIFOs implemented?
//
static Bool enableFIFIOs(or1kP or1k) {
    return OR1K_ENABLE_FIFOS((vmiProcessorP)or1k);
}
...
//
// Get from connection, don't block if data not available, return status
//
static void vmic_ConnPut(or1kP or1k) {
    or1k->carryFlag = vmirtConnPut(or1k->outputConn, &or1k->connValue);
}

//
// Put to connection, block if space not available
//
...
//
// Put to connection, don't block if space not available, write success code
// into carry flag
//
...
// Get from connection, don't block if data not available, return status
//
static Uns32 vmic_ConnGet(or1kP or1k, Bool peek) {
    or1k->carryFlag = vmirtConnGet(or1k->inputConn, &or1k->connValue, peek);
    return or1k->connValue;
}
...
//
// Get from connection, don't block if data not available, write success code
// into carry flag
//
...
static void vmic_MTSPR_SM(or1kP or1k, Uns32 ra, Uns32 rb, Uns32 k) {
    ...
    switch(sprNum) {
        ...
        case SPR_OFF(SPR_SYS,SYS_FIFO_B):
            if(enableFIFIOs(or1k)) {
                or1k->connValue = rb;
                vmic_ConnPutOrBlock(or1k);
            } else {
                ignoreMTSPR(or1k, sprNum);
            }
            break;

        case SPR_OFF(SPR_SYS,SYS_FIFO_NB):
            if(enableFIFIOs(or1k)) {
                or1k->connValue = rb;
                vmic_ConnPut(or1k);
            } else {
...
static Uns32 vmic_MFSPR_SM(or1kP or1k, Uns32 ra, Uns32 k) {
    ...
    switch(sprNum) {
        ...
        case SPR_OFF(SPR_SYS,SYS_FIFO_B):
            if(enableFIFIOs(or1k)) {
                result = vmic_ConnGetOrBlock(or1k, False);
            } else {
        ...
        case SPR_OFF(SPR_SYS,SYS_FIFO_NB):
            if(enableFIFIOs(or1k)) {
                result = vmic_ConnGet(or1k, False);
            } else {
                result = ignoreMFSPR(or1k, sprNum);
            }
            break;

        case SPR_OFF(SPR_SYS,SYS_PEEK_B):
            if(enableFIFIOs(or1k)) {
                result = vmic_ConnGetOrBlock(or1k, True);
            } else {
                result = ignoreMFSPR(or1k, sprNum);
            }
            break;

        case SPR_OFF(SPR_SYS,SYS_PEEK_NB):
            if(enableFIFIOs(or1k)) {
                result = vmic_ConnGet(or1k, True);
            } else {
                result = ignoreMFSPR(or1k, sprNum);
            }
            break;
...
static OR1K_MORPH_FN(morphNOP) {
    ...
    Uns32 nopType = state->info.c;

    // use this NOP to get ucLinux to exit
    if(nopType==5) {
        vmimtFinish();
    }
}
...
VMI_MORPH_FN(or1kMorphInstruction) {
    ...
    // add any explicit instruction class
