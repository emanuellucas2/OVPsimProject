...
static hashTableP hash64TableCreate(Uns32 size)
{
...
// Return the hash if found, create one otherwise
//
static hashEntryP hash64FindAdd(hashTableP table, Uns64 key)
{
...
//
static hashEntryP hash64Find(hashTableP table, Uns64 key)
{
    Uns64 hash = hashFunc(key);
...
//
static void *hash64GetValue(hashEntryP entry)
{
    return entry->value;
}

static void hash64SetValue(hashEntryP entry, void *v)
{
    entry->value = v;
}
...
void deltaWrite(hashTableP table, Uns64 sector, const Uns8 *buffer)
{
    ...
    if (n) {
        ...
        if(DEBUG) {
            bhmMessage(
                "I", PREFIX,
                "deltaWrite sector:%lld",
                sector
            );
        }
    } else {
        bhmMessage("F", PREFIX, "Failed to create delta");
    }
}
