...
//
// Define the op object structure
//
...
static VMIOS_CONSTRUCTOR_FN(constructor) {
    ...
    // return register (standard ABI)
    object->resultReg = vmiosGetRegDesc(processor, "R11");

    // stack pointer (standard ABI)
    object->sp = vmiosGetRegDesc(processor, "R1");

    // Stack Pointer : TODO OP does not allow this to work
...
//
// Intercept open()
// Note that the newlib declares open using varargs and the or1k compiler does not
// seem to place arguments in registers for varargs functions. Thus we need to implement
// a special version of the intercept of open() that gets the args from the stack.
//
...
vmiosAttr modelAttrs = {
    ...
    .intercepts = {
        ...
        {  "_open",          0,      True,   openOr1k       },  // Non-standard
        {  "_read",          0,      True,   readCB         },
        {  "_stat",          0,      True,   statCB         },
        {  "_time",          0,      True,   timeCB         },
