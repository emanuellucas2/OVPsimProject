...
static const char *opcode[MICROBLAZE_IT_LAST+1] = {
    ...
    [MICROBLAZE_IT_SWAPB_TYPEAB] = "swapb",
    [MICROBLAZE_IT_SWAPH_TYPEAB] = "swaph",
    ...
    //
    // Instruction Opcode TypeBbe Insns
    //
    [MICROBLAZE_IT_BSEFI_TYPEBBE] = "bsefi",

    //
    // Instruction Opcode TypeBbi Insns
    //
    [MICROBLAZE_IT_BSIFI_TYPEBBI] = "bsifi",
...
// ISA_V8_20 Decoder
vmidDecodeTableP microblazeCreateDecodeTable_ISA_V8_20_32Bit(vmidDecodeTableP current) {
    vmidDecodeTableP table = current;
    if (!table) {
        table = vmidNewDecodeTable(32, MICROBLAZE_IT_LAST);
    }

    //                         TypeA1   |opcode|   rD|   rA|   rB|     subop1|
    ...
    //                         TypeA2   |opcode|D|opco|   rA|   rB|     subop1|
    ...
    //                         TypeA3   |opcode|   rD|D|A| c2|   rB|     subop1|
    ...
    //                         TypeA7   |opcode|   rD|   rA|   rB|   opcode|U|o|
    ...
    //                         TypeA8   |opcode|   rD|   rA|   rB|z|R|    zero1|
    ...
    DECODE_ENTRY(22,     SWAPB_TYPEAB, "|100100|.....|.....|0000000111100000|");
    DECODE_ENTRY(22,     SWAPH_TYPEAB, "|100100|.....|.....|0000000111100010|");

    //                         TypeAc   |opcode|zero1|   rA|   rB|     subop1|
    ...
    //                        TypeBbe   |opcode|   rD|   rA|IE| c2| immw|b| imms|
    DECODE_ENTRY(12,    BSEFI_TYPEBBE, "|011001|.....|.....|01|000|.....|0|.....|");

    //                        TypeBbi   |opcode|   rD|   rA|IE| c2| immw|b| imms|
    DECODE_ENTRY(12,    BSIFI_TYPEBBI, "|011001|.....|.....|10|000|.....|0|.....|");

    //                         TypeC1   |opcode|   rD| zero1|nctae| zero1|FSLx|
    ...
    //                         TypeC2   |opcode|   rD|zero1|   rB|z|nctae|zero1|
    ...
    //                         TypeC3   |opcode|init1|   rA|i|ncta|  init1|FSLx|
    ...
    //                        TypeC3d   |opcode|zero1|   rA|   rB|z|ncta| zero1|
    ...
    //                         TypeC4   |opcode|init1|   rA|i|ncta|  init1|FSLx|
    ...
    //                        TypeC4d   |opcode|zero1|   rA|   rB|z|ncta| zero1|
...
//
void microblazeGenInstructionInfo (microblazeInstructionInfoP info) {
    ...
    switch (info->type) {
        ...
        case MICROBLAZE_IT_XOR_TYPEA1:
            ...
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            break;
        ...
        case MICROBLAZE_IT_MFS_TYPEA9:
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            info->rS = VECINDEX(info->instruction,13,0,0); // 13:0
            break;
        ...
        case MICROBLAZE_IT_SWAPB_TYPEAB:
        case MICROBLAZE_IT_SWAPH_TYPEAB:
            info->rA = VECINDEX(info->instruction,20,16,0); // 20:16
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            break;
        ...
        case MICROBLAZE_IT_XORI_TYPEB1:
            info->rA = VECINDEX(info->instruction,20,16,0); // 20:16
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            info->simm = VECINDEX(info->instruction,15,0,0); // 15:0
        ...
        case MICROBLAZE_IT_MBAR_TYPEB8:
            info->simm = VECINDEX(info->instruction,25,21,0); // 25:21
        ...
        case MICROBLAZE_IT_MSRSET_TYPEB9:
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            info->uimm = VECINDEX(info->instruction,14,0,0); // 14:0
            break;
        ...
        case MICROBLAZE_IT_BSEFI_TYPEBBE:
            info->imms = VECINDEX(info->instruction,4,0,0); // 4:0
            info->immw = VECINDEX(info->instruction,10,6,0); // 10:6
            info->rA = VECINDEX(info->instruction,20,16,0); // 20:16
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            break;

        case MICROBLAZE_IT_BSIFI_TYPEBBI:
            info->imms = VECINDEX(info->instruction,4,0,0); // 4:0
            info->immw = VECINDEX(info->instruction,10,6,0); // 10:6
            info->rA = VECINDEX(info->instruction,20,16,0); // 20:16
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
        ...
        case MICROBLAZE_IT_TNGET_TYPEC1:
            info->FSLx = VECINDEX(info->instruction,3,0,0); // 3:0
            info->rD = VECINDEX(info->instruction,25,21,0); // 25:21
            break;
        ...
        case MICROBLAZE_IT_IMM_TYPEI1:
            info->simmhi = VECINDEX(info->instruction,15,0,0); // 15:0
