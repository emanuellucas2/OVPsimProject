...
#define EMITIMMW       "\213"
#define EMITIMMW_CASE  '\213'
#define EMITIMMS       "\214"
#define EMITIMMS_CASE  '\214'
#define EMITWIDTH      "\215"
#define EMITWIDTH_CASE '\215'
...
const char *formats[MICROBLAZE_IT_LAST+1] = {
    ...
    [MICROBLAZE_IT_SWAPB_TYPEAB]  = EMITOP " " EMITRD ", " EMITRA,
    [MICROBLAZE_IT_SWAPH_TYPEAB]  = EMITOP " " EMITRD ", " EMITRA,
    ...
    //
    // Instruction Format TypeBbe Insns
    //
    [MICROBLAZE_IT_BSEFI_TYPEBBE] = EMITOP " " EMITRD ", " EMITRA ", " EMITIMMW ", " EMITIMMS,

    //
    // Instruction Format TypeBbi Insns
    //
    [MICROBLAZE_IT_BSIFI_TYPEBBI] = EMITOP " " EMITRD ", " EMITRA ", " EMITWIDTH ", " EMITIMMS,
...
//
// Function to format the info field #immw#
//
static void EMITIMMW_FN(char **result, microblazeInstructionInfoP info) {
    char tmp[32];
    const char *fmt = "%u";
    sprintf(tmp, fmt, info->immw);
    putString(result, tmp);
}

//
// Function to format the info field #imms#
//
static void EMITIMMS_FN(char **result, microblazeInstructionInfoP info) {
    char tmp[32];
    const char *fmt = "%u";
    sprintf(tmp, fmt, info->imms);
    putString(result, tmp);
}

//
// Function to format the info field width from #imms# #immw#
//
static void EMITWIDTH_FN(char **result, microblazeInstructionInfoP info) {
    char tmp[32];
    const char *fmt = "%u";
    Uns32 width = info->immw - info->imms + 1;
...
static void disassembleFormat(
    ...
    while((ch=*format++)) {
         switch(ch) {
             ...
             case EMITIMMW_CASE:
                 EMITIMMW_FN(result, info);
                 break;

             case EMITIMMS_CASE:
                 EMITIMMS_FN(result, info);
                 break;

             case EMITWIDTH_CASE:
                 EMITWIDTH_FN(result, info);
                 break;
