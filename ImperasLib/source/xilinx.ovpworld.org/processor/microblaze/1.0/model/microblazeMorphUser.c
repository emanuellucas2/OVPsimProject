...
static memEndian getEndian(microblazeMorphStateP state) {
    memEndian endian;
    endian = (state->microblaze->SPR_PVR0.ENDI == 0) ? MEM_ENDIAN_BIG : MEM_ENDIAN_LITTLE;
    return endian;
}
...
static Bool validatePrivMode(microblazeP microblaze) {
    ...
    if (microblaze->SPR_MSR.UM) {
        TYPE_SPR_ESR ESR = {0};
        ESR.EC  = PRIVILEGED_INSTRUCTION_EXCEPTION;
        ESR.ESS = 0;
...
static Bool validateFSLPrivMode(microblazeP microblaze) {
    vmimtValidateBlockMask(BM_MSR_UM);
    if (microblaze->SPR_MSR.UM && microblazeMMUPrivileged(microblaze)) {
        TYPE_SPR_ESR ESR = {0};
        ESR.EC  = PRIVILEGED_INSTRUCTION_EXCEPTION;
        ESR.ESS = 0;
        emit_hwExceptionConst(ESR);
...
MICROBLAZE_MORPH_FN(morphADD_TYPEA1) {
    ...
    if (rD == 0 && rA == 0 && rB == 0) {
        ...
        //
        if (state->microblaze->SPR_PVR2.OP0EXC) {
            TYPE_SPR_ESR ESR = {0};
...
//
// Dependant upon
// C_M_AXI_DP_EXCLUSIVE_ACCESS (Data Peripheral)
// C_M_AXI_DC_EXCLUSIVE_ACCESS (Data Cache)
//
MICROBLAZE_MORPH_FN(morphLWX_TYPEA1) {
    ...
    //
    vmimtMoveRC(MICROBLAZE_RESERVATION_BITS,        MICROBLAZE_RESERVATION_WR, 1);
    vmimtMoveRR(MICROBLAZE_RESERVATION_ADDR_BITS,   MICROBLAZE_RESERVATION_ADDR_WR,   MICROBLAZE_TMP_RD(1));
    vmimtMoveRC(MICROBLAZE_RESERVATION_LENGTH_BITS, MICROBLAZE_RESERVATION_LENGTH_WR, 4);

    //
    // Clear MSR[C] - this needs to be conditional on an AXI4 interface being present
    // C_M_AXI_Dx_EXCLUSIVE_ACCESS
    //
    Uns8 EXOKAY = 1;

    if (state->microblaze->configInfo.arch <= V8_20){
        //
        // Upto and including ISA V8.20
        //
        EXOKAY = 0;

    } else if (state->microblaze->SPR_PVR2.AXI4DP &&
               state->microblaze->SPR_PVR5.AXI4DC) {
        //
        // Beyond ISA V8.20
        //
    ...
    vmimtMoveRC(MICROBLAZE_FLG_CC_BITS, MICROBLAZE_FLG_CC_WR, EXOKAY);
...
MICROBLAZE_MORPH_FN(morphSWX_TYPEA1) {
    ...
    //    MSR[C] = 0
    // endif
    vmimtCompareRC(MICROBLAZE_FLG_TMP_BITS, vmi_COND_EQ, MICROBLAZE_RESERVATION_RD, 0, MICROBLAZE_FLG_TMP_WR);
...
static void vmic_BRK_TYPEA5(microblazeP microblaze) {
    microblaze->SPR_MSR.BIP = 1;
    microblaze->reservation = 0;
}
...
static void morphUR_mask(microblazeMorphStateP state, Uns32 mask) {
    //
    // Supported Architecture=MICROBLAZE_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtMoveRC(MICROBLAZE_GPR_BITS, MICROBLAZE_TMP_WR(0), rD);
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtBinopRC(MICROBLAZE_TMP_BITS, vmi_AND, MICROBLAZE_TMP_WR(1), 0xFFFFFFFC, 0); // Word-Align

    vmimtLoadRRO(MICROBLAZE_GPR_BITS, MICROBLAZE_GPR_BITS, 0, MICROBLAZE_TMP_WR(2), MICROBLAZE_TMP_RD(1), invertEndian(getEndian(state)), False, True);

    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtBinopRC(MICROBLAZE_TMP_BITS,  vmi_AND, MICROBLAZE_TMP_WR(1), 0x3, 0); // bottom 2 address bits
    vmimtBinopRC(MICROBLAZE_TMP_BITS,  vmi_MUL, MICROBLAZE_TMP_WR(1), 8, 0); // shift value addr[1:0]*8
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_SHR, gpr_rD, MICROBLAZE_TMP_RD(2), MICROBLAZE_TMP_RD(1), 0); // mem >> 8/16/24
    vmimtBinopRC(MICROBLAZE_TMP_BITS,  vmi_AND, gpr_rD, mask, 0); // Mask to byte
}

MICROBLAZE_MORPH_FN(morphLBUR_TYPEA8) {
    morphUR_mask(state, 0xFF);
}

MICROBLAZE_MORPH_FN(morphLHUR_TYPEA8) {
    morphUR_mask(state, 0xFFFF);
}

static void morphLW_endian(microblazeMorphStateP state, memEndian endian) {
    //
    // Supported Architecture=MICROBLAZE_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtMoveRC(MICROBLAZE_GPR_BITS, MICROBLAZE_TMP_WR(0), rD);
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtLoadRRO(MICROBLAZE_GPR_BITS, MICROBLAZE_GPR_BITS, 0, gpr_rD, MICROBLAZE_TMP_RD(1), endian, False, True);
}

MICROBLAZE_MORPH_FN(morphLW_TYPEA8) {
    morphLW_endian(state, getEndian(state));
}

MICROBLAZE_MORPH_FN(morphLWR_TYPEA8) {
    morphLW_endian(state, invertEndian(getEndian(state)));
}

static void morphSB_endian(microblazeMorphStateP state, memEndian endian) {
    //
    // Supported Architecture=MICROBLAZE_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtMoveRC(MICROBLAZE_GPR_BITS, MICROBLAZE_TMP_WR(0), rD);
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtStoreRRO(MICROBLAZE_GPR_BITS/4, 0, MICROBLAZE_TMP_RD(1), gpr_rD, endian, True);
}

MICROBLAZE_MORPH_FN(morphSB_TYPEA8) {
    morphSB_endian(state, getEndian(state));
}

MICROBLAZE_MORPH_FN(morphSBR_TYPEA8) {
    morphSB_endian(state, invertEndian(getEndian(state)));
}

static void morphSH_endian(microblazeMorphStateP state, memEndian endian) {
    //
    // Supported Architecture=MICROBLAZE_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtMoveRC(MICROBLAZE_GPR_BITS, MICROBLAZE_TMP_WR(0), rD);
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtStoreRRO(MICROBLAZE_GPR_BITS/2, 0, MICROBLAZE_TMP_RD(1), gpr_rD, endian, True);
}

MICROBLAZE_MORPH_FN(morphSH_TYPEA8) {
    morphSH_endian(state, getEndian(state));
}

MICROBLAZE_MORPH_FN(morphSHR_TYPEA8) {
    morphSH_endian(state, invertEndian(getEndian(state)));
}

static void morphSW_endian(microblazeMorphStateP state, memEndian endian) {
    //
    // Supported Architecture=MICROBLAZE_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtMoveRC(MICROBLAZE_GPR_BITS, MICROBLAZE_TMP_WR(0), rD);
    vmimtBinopRRR(MICROBLAZE_TMP_BITS, vmi_ADD, MICROBLAZE_TMP_WR(1), gpr_rA, gpr_rB, 0);
    vmimtStoreRRO(MICROBLAZE_GPR_BITS, 0, MICROBLAZE_TMP_RD(1), gpr_rD, endian, True);
}

MICROBLAZE_MORPH_FN(morphSW_TYPEA8) {
    morphSW_endian(state, getEndian(state));
}

MICROBLAZE_MORPH_FN(morphSWR_TYPEA8) {
    morphSW_endian(state, invertEndian(getEndian(state)));
}
...
static void vmic_MTS_TYPEAA(microblazeP microblaze, Uns32 rS, Uns32 rA) {
    ...
    switch (rS) {
        ...
        case 0x1000 :
            MTSREG(PID, rA);
            vmirtSetProcessorASID((vmiProcessorP)microblaze, microblaze->SPR_PID.PID);
            break;
        case 0x1001 : {
            Uns32 oldZPR = microblaze->SPR_ZPR.reg;
            MTSREG(ZPR, rA);
            if (oldZPR != microblaze->SPR_ZPR.reg) {
                microblazeTLBFlush(microblaze);
            }
            break;
        }
        case 0x1002 :
...
MICROBLAZE_MORPH_FN(morphSWAPB_TYPEAB) {
    //
    // Supported Architecture=ISA_V8_20
    //
     Uns8 rA = state->info.rA;
     vmiReg gpr_rA = MICROBLAZE_GPR_RD(rA);
     Uns8 rD = state->info.rD;
     ...
     vmimtUnopRR(MICROBLAZE_GPR_BITS, vmi_SWP, gpr_rD, gpr_rA, 0);
}

MICROBLAZE_MORPH_FN(morphSWAPH_TYPEAB) {
    //
    // Supported Architecture=ISA_V8_20
    //
    Uns8 rA = state->info.rA;
    ...
    vmimtBinopRRC(MICROBLAZE_GPR_BITS, vmi_ROR, gpr_rD, gpr_rA, 16, 0);
}
...
//
static Uns32 vmic_BSEFI_TYPEBBI(microblazeP microblaze, Uns32 imms, Uns32 immw, Uns32 rA, Uns32 rD) {
    Uns32 rA1  = rA >> imms;
    Uns32 mask = 0xffffffff >> (32 - immw);

    Uns32 rD1 = mask & rA1;

    if (immw<1 || immw>31) {
        vmiPrintf("Error: bsefi immw = %d\n", immw);
        rD1 = rD;
    }
    if ((immw + imms) > 32) {
        vmiPrintf("Error: bsefi immw+imms = %d\n", immw+imms);
        rD1 = rD;
    }
    return rD1;
}

MICROBLAZE_MORPH_FN(morphBSEFI_TYPEBBE) {
    //
    // Supported Architecture=ISA_V8_20
    //
    Uns8 imms = state->info.imms;
    Uns8 immw = state->info.immw;
    Uns8 rA = state->info.rA;
    vmiReg gpr_rA = MICROBLAZE_GPR_RD(rA);
    Uns8 rD = state->info.rD;
    vmiReg gpr_rD = MICROBLAZE_GPR_WR(rD);
    ...
    vmimtArgUns32(immw);
    vmimtArgReg(MICROBLAZE_GPR_BITS, gpr_rA);
    vmimtArgReg(MICROBLAZE_GPR_BITS, gpr_rD);
    vmimtCallResult((vmiCallFn)vmic_BSEFI_TYPEBBI, 32, gpr_rD);
}

//
// mask <- (0xffffffff << (IMMW + 1)) _XOR_ (0xffffffff << IMMS)
// (rD) <- ((rA) << IMMS) _AND_ mask) _OR_ ((rD) _AND_ mask)
//
static Uns32 vmic_BSIFI_TYPEBBI(microblazeP microblaze, Uns32 imms, Uns32 immw, Uns32 rA, Uns32 rD) {
    Uns32 t1 = 0xffffffff;
    Uns32 t2 = 0xffffffff;

    Uns8 shift = immw + 1;

    t1 = (shift==32) ? 0 : t1<<shift;

    t2 <<= imms;
    Uns32 mask = t1 ^ t2;

    Uns32 rD1 = ((rA << imms) & mask) | (rD & ~mask);

    if (immw <= imms) {
        vmiPrintf("Error: bsifi immw(%d) <= imms(%d)\n", immw, imms);
        rD1 = rD;
    }
...
MICROBLAZE_MORPH_FN(morphBSIFI_TYPEBBI) {
    //
    // Supported Architecture=ISA_V8_20
    //
    Uns8 imms = state->info.imms;
    ...
    vmimtArgReg(MICROBLAZE_GPR_BITS, gpr_rA);
    vmimtArgReg(MICROBLAZE_GPR_BITS, gpr_rD);
    vmimtCallResult((vmiCallFn)vmic_BSIFI_TYPEBBI, 32, gpr_rD);
}
...
static Bool Morph_MSR_SET_CLR_TYPEB9(Uns32 rD, Uns32 imm, Uns32 set) {
    ...
    switch (imm) {
        ...
        case (MICROBLAZE_MSR_UMS | MICROBLAZE_MSR_VMS):
            vmimtCall((vmiCallFn)vmic_setclr_ums_vms);
            Status = True;
            break;
...
static void emit_get(microblazeMorphStateP state, Bool n, Bool c, Bool t, Bool a, Bool e, Bool d) {
    ...
    if (n) {
        ...
        // blocking
        vmimtConnGetRB(MICROBLAZE_FSL_BITS, MICROBLAZE_TMP_WR(0), SFSL, peek, 0);
...
static void emit_put(microblazeMorphStateP state, Bool n, Bool c, Bool t, Bool a, Bool d) {
    ...
    if (n) {
        ...
        // blocking
        vmimtConnPutRB(MICROBLAZE_FSL_BITS, MFSL, SOURCE_REG, 0);
    }
...
VMI_SAVE_STATE_FN(microblazeSaveStateCB) {
    ...
    switch(phase) {
        ...
        case SRT_END_CORE:
            ...
            VMIRT_SAVE_FIELD(cxt, microblaze, InterruptActive);
            // Save TLB
            vmirtSave(cxt, "TLB", microblaze->utlb.entry, (sizeof(TLBentry) * microblaze->utlb.size));
...
VMI_RESTORE_STATE_FN(microblazeRestoreStateCB) {
    ...
    switch(phase) {
        ...
        case SRT_END_CORE:
            // end of individual core
            VMIRT_RESTORE_FIELD(cxt, microblaze, InterruptActive);
            // Restore TLB
            microblazeTLBFlush(microblaze);
            vmirtRestore(cxt, "TLB", microblaze->utlb.entry, (sizeof(TLBentry) * microblaze->utlb.size));
            microblazeTLBPrivReset(microblaze);
