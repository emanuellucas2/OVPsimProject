...
void microblazeDoc(vmiProcessorP processor, microblazeParamValuesP parameters) {
    ...
    vmidocAddText(Features, "Instruction Set: This model fully implements the instruction set upto and including V10.");
    ...
    vmidocAddText(lim,
        "GDB: There is a known issue in the microblaze gdb, "
        "a lockup can occur when disassembling from address 0");
    ...
    if (arch >= V9_50) {
        vmidocAddText(cfgfeatures, "Load/Store Exclusive over AXI4");
    }
