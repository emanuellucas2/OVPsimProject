...
static VMI_REG_READ_FN(readStop) {
    microblazeP microblaze = (microblazeP) processor;
    *(Uns32*)buffer = (microblaze->disable & MBD_STOP) && True;
    return True;
}
...
static VMI_REG_WRITE_FN(writeStop) {
    ...
    if(*(Uns32*)buffer) {
        microblazeHalt(microblaze, MBD_STOP);
    } else {
        microblazeRestart(microblaze, MBD_STOP);
    }
    return True;
}
...
static const vmiRegGroup groups[MICROBLAZE_RG_LAST+1] = {
    ...
    [MICROBLAZE_RG_INTEGRATION] = {name: "Integration_support" },
};
...
static const vmiRegInfo registers[] = {
    MICROBLAZE_REG_INFO("R0",         0, vmi_REG_LR,   32, &groups[MICROBLAZE_RG_USER],   MICROBLAZE_RAW_REG_R(gpr[0])),
    ...
    MICROBLAZE_REG_INFO("SPR_PVR0",  38, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR0)),
    MICROBLAZE_REG_INFO("SPR_PVR1",  39, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR1)),
    MICROBLAZE_REG_INFO("SPR_PVR2",  40, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR2)),
    MICROBLAZE_REG_INFO("SPR_PVR3",  41, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR3)),
    MICROBLAZE_REG_INFO("SPR_PVR4",  42, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR4)),
    MICROBLAZE_REG_INFO("SPR_PVR5",  43, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR5)),
    MICROBLAZE_REG_INFO("SPR_PVR6",  44, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR6)),
    MICROBLAZE_REG_INFO("SPR_PVR7",  45, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR7)),
    MICROBLAZE_REG_INFO("SPR_PVR8",  46, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR8)),
    MICROBLAZE_REG_INFO("SPR_PVR9",  47, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR9)),
    MICROBLAZE_REG_INFO("SPR_PVR10", 48, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR10)),
    MICROBLAZE_REG_INFO("SPR_PVR11", 49, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_R(SPR_PVR11)),
...
//    MICROBLAZE_REG_INFO("tmp3", 62, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_SYSTEM], MICROBLAZE_RAW_REG_RW(tmp[3])),
    ...
    //
    // integration support registers
    //
    MICROBLAZE_REG_INFO("stop", SUPPORT_REGS+0, vmi_REG_NONE, 32, &groups[MICROBLAZE_RG_INTEGRATION], MICROBLAZE_CB_REG_RW(readStop, writeStop)),
...
VMI_REG_INFO_FN(microblazeRegInfo) {
    ...
    while (!next) {
        //
        // Get the next candidate or return NULL if no more registers
        //
        if(!prev) {
            next = registers;
        } else if((prev+1)->name) {
        ...
        //
        // if gdb registers are required skip these registers
        //
        if (gdbFrame && (next->gdbIndex >= SUPPORT_REGS)) {
            prev = next;
            next = 0;
        }
    }
