...
static VMI_NET_CHANGE_FN(signalReset) {
    ...
    if (posedge(microblaze->Reset, newValue)) {
        ...
        microblazeHalt(microblaze, MBD_RESET);

    } else {
        // restart the processor now reset is inactive

        microblaze->ResetPending = False;

        microblazeProcessReset(processor);

        microblazeRestart(microblaze, MBD_RESET);
...
void microblazeNewNetPorts(microblazeP microblaze) {
    microblazeNetPort template[] = {
        { {name : "Interrupt",  type : vmi_NP_INPUT, netChangeCB : signalInterrupt },  V7_00 },
        { {name : "Reset",      type : vmi_NP_INPUT, netChangeCB : signalReset     },  V7_00 },
        { {name : "MB_Reset",   type : vmi_NP_INPUT                                },  V7_00 },
        { {name : "Ext_BRK",    type : vmi_NP_INPUT                                },  V7_00 },
        { {name : "Ext_NM_BRK", type : vmi_NP_INPUT                                },  V7_00 },
        { { 0 } }
    };
