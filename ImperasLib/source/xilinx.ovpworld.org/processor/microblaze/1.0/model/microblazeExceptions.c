...
static void commonException(microblazeP microblaze, Bool MMU_exception) {
    ...
    vmirtSetPCException((vmiProcessorP)microblaze, (microblaze->SPR_PVR12.VECTORS + HARDWARE_EXCEPTION_VECTOR));
}
...
void microblazeTLBMissException(
    ...
    if(action==TLBA_FETCH) {
        microblaze->SPR_ESR.EC = INSTRUCTION_TLB_MISS_EXCEPTION;
        microblaze->exception = microblaze_E_INSTRUCTION_TLB_MISS_EXCEPTION;
    } else {
        microblaze->SPR_ESR.EC = DATA_TLB_MISS_EXCEPTION;
...
void microblazeStreamExceptionCB(vmiProcessorP processor, Uns32 insn, Uns32 e, Uns32 d, Uns32 fsl, Uns32 rB, Uns32 rD) {
    ...
    if (microblaze->SPR_MSR.EE) {
        ...
        if (e) {
            ...
            microblaze->SPR_ESR.EC = STREAM_EXCEPTION;
            ...
            //
            microblaze->SPR_EAR.reg = 0xdeadea00 | microblaze->SPR_ESR.EC;
...
static void machineAlignError(vmiProcessorP processor, Uns32 bytes, Uns32 rDidx, Uns32 isWrite) {
    ...
    microblaze->SPR_ESR.EC = UNALIGNED_DATA_ACCESS;
...
//      ESR[EC]=00001; ESR[W]=0; ESR[S]=0; ESR[Rx]=rD
    ...
    if        (TLB_Miss && microblaze->SPR_MSR.VM) {
    } else if (Access_Protected && microblaze->SPR_MSR.UM && microblaze->SPR_MSR.VM) {
    } else {
...
//    Uns32 R2 : 1;   // (reserved for internal use)
        ...
        case VMI_INTEGER_DIVIDE_BY_ZERO:
            ...
            microblaze->SPR_MSR.DZO = 1;

            if (microblaze->SPR_PVR2.DIVEXC && microblaze->SPR_MSR.EE) {
                //
...
VMI_FP_ARITH_EXCEPT_FN(microblazeFPArithExcept) {
    ...
    if       (flags->f.I) {
        ...
        // invalid operation
        //
        microblaze->SPR_FSR.IO = 1;
    } else if(flags->f.D) {
        //
        // denormal flag
        //
        microblaze->SPR_FSR.DO = 1;
    } else if(flags->f.Z) {
        //
        // divide-by-zero flag
        //
        microblaze->SPR_FSR.DZ = 1;
    } else if(flags->f.O) {
        //
        // overflow flag
        //
        microblaze->SPR_FSR.OF = 1;
    } else if(flags->f.U) {
        //
        // underflow flag
        //
        microblaze->SPR_FSR.UF = 1;
    } else {
        //
...
static Bool isIntActive(microblazeP microblaze) {
    ...
    //
    Bool irqedge   = microblaze->SPR_PVR2.IRQEDGE;
    Bool irqpos    = microblaze->SPR_PVR2.IRQPOS;
    Bool ie        = microblaze->SPR_MSR.IE;
    Bool eip       = microblaze->SPR_MSR.EIP;
    Bool bip       = microblaze->SPR_MSR.BIP;
...
void microblazeProcessReset(vmiProcessorP processor) {
    ...
    vmirtSetPCException((vmiProcessorP)microblaze, (microblaze->SPR_PVR12.VECTORS + RESET_VECTOR));
}
...
void microblazeProcessInterrupt(vmiProcessorP processor) {
    ...
    vmirtSetPCException((vmiProcessorP)microblaze, (microblaze->SPR_PVR12.VECTORS + INTERRUPT_VECTOR));
...
void microblazeRegisterInit (microblazeP microblaze) {
    ...
    microblaze->SPR_MSR.PVR = (microblaze->params.C_PVR) ? 1 : 0;
...
//
void microblazeHalt(microblazeP microblaze, microblazeDisable reason) {
    microblaze->disable |= reason;
...
//
void microblazeRestart(microblazeP microblaze, microblazeDisable reason) {
    microblaze->disable &= ~(reason);
    if (microblaze->disable == MBD_ACTIVE) vmirtRestartNext((vmiProcessorP)microblaze);
}
