...
PPM_READ_CB(rxFIFOId) {
    if(PSE_DIAG_LOW)
        bhmMessage("W", PREFIX "_FIFOID", "No Action");
    return 0;
}
...
PPM_REG_WRITE_CB(wrHPBId) {
    if(PSE_DIAG_LOW)
        bhmMessage("W", PREFIX "_HPBID", "No Action");
}

PPM_CONSTRUCTOR_CB(constructor) {
    if(PSE_DIAG_LOW)
        bhmMessage("W", PREFIX, "This model implements only the registers. There is no behavior.");
