...
class riscv_A70XP_MP : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "A70XP-MP");
        return p;
    }
    ...
    const char *getModel() {
        return opVLNVString (NULL, "codasip.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, OP_VLNV_FATAL);
    }
  ...
  public:
    ...
    riscv_A70XP_MP(tlmModule &parent, sc_module_name name)
    ...
    riscv_A70XP_MP(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=56, Uns32 DATAWidth=56)
