...
//
void Nios_IIHalt(Nios_IIP Nios_II, Nios_IIDisable reason) {
    Nios_II->disable |= reason;
...
//
void Nios_IIRestart(Nios_IIP Nios_II, Nios_IIDisable reason) {
    Nios_II->disable &= ~(reason);
    if (Nios_II->disable == NIOS_II_ACTIVE) vmirtRestartNext((vmiProcessorP)Nios_II);
}
