...
//
#define NIOS_II_REG_INFO(_NAME, _INDEX, _USAGE, _BITS, _GROUP, _DETAIL) { \
    name     : _NAME,    \
    gdbIndex : _INDEX,   \
    usage    : _USAGE,   \
    bits     : _BITS,    \
...
#define NIOS_II_CB_REG_RW(_RCB, _WCB) \
    access   : vmi_RA_RW,           \
    readCB   : _RCB,                \
    writeCB  : _WCB
...
static VMI_REG_READ_FN(readStop) {
    ...
    *(Uns32*)buffer = (Nios_II->disable & NIOS_II_STOP) && True;
    return True;
}
...
static VMI_REG_WRITE_FN(writeStop) {
    ...
    if(*(Uns32*)buffer) {
        Nios_IIHalt(Nios_II, NIOS_II_STOP);
    } else {
        Nios_IIRestart(Nios_II, NIOS_II_STOP);
    }
    return True;
}
...
static const vmiRegGroup groups[NIOS2_RG_LAST+1] = {
    ...
    [NIOS2_RG_INTEGRATION] = {name: "Integration_support" },
};
...
static const vmiRegInfo registers[] = {
    ...
    {name:"ra",       gdbIndex:31, usage:vmi_REG_LR,   bits:32, access:vmi_RA_RW, group:&groups[NIOS2_RG_USER],   raw:NIOS_II_CPU_REG(gpr[31])},
    ...
    //
    NIOS_II_REG_INFO("stop", SUPPORT_REGS+0, vmi_REG_NONE, 32, &groups[NIOS2_RG_INTEGRATION], NIOS_II_CB_REG_RW(readStop, writeStop)),
