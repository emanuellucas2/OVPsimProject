...
int main(int argc, const char *argv[]) {
    ...
    if (options.loadprocessor ) {
        ...
        opProcessorApplicationLoad(
            proc, options.application, OP_LDR_VERBOSE|OP_LDR_FAIL_IS_ERROR|OP_LDR_SET_START, 0
        );
        ...
        thisApp = opMemoryApplicationLoad(
                    mem, options.application, OP_LDR_VERBOSE|OP_LDR_FAIL_IS_ERROR, 0
                  );
        ...
        thisApp = opBusApplicationLoad(
                    bus, options.application, OP_LDR_VERBOSE|OP_LDR_FAIL_IS_ERROR, 0
                  );
