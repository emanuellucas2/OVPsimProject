...
//
FUNCTOHOST(Uns32, initSemiHost, void *addressRegisters, Uns32 diag)

//
// Semihosted function: performs the DMA using native code for a configured channel
//
FUNCTOHOST(Uns32, transferDataNative, Uns32 src, Uns32 dest, Uns32 thisAccess)
...
static void dmaBurst(Uns32 ch)
{
    ...
    while (bytes) {
        ...
        if(readNativeDataChannel) {
            // Perform a transfer using native code
            if (PSE_DIAG_HIGH) {
                bhmMessage("I", PREFIX "_DMASH", "Using semihost burst %u bytes", thisAccess);
            }
...
void userInit(void)
{
    ...
    if(readNativeDataChannel) {
        initSemiHost(&DMACSP_ab32_data[0], PSE_DIAG_SEMIHOST);
    }
}
