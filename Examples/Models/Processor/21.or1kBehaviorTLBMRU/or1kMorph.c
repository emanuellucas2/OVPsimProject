...
//
// Emit code for OR1K nop instruction
//
...
static OR1K_MORPH_FN(morphNOP) {
    ...
    Uns32 code = state->info.c;

    // subvert nop 98 to print the contents of r1 and nop 99 to set a TLB entry
    if(code==98) {
        vmimtArgReg(OR1K_BITS, getGPR(1));
        vmimtCall((vmiCallFn)vmic_printVal);
    } else if(code==99) {
        vmimtArgProcessor();
        vmimtArgReg(OR1K_BITS, getGPR(1));    // r1: TLB entry index
        vmimtArgReg(OR1K_BITS, getGPR(2));    // r2: virtual address (va)
        vmimtArgReg(OR1K_BITS, getGPR(3));    // r3: physical address (pa)
