...
VMI_VMINIT_FN(or1kVMInit) {
    ...
    // initialise MRU state tables and current state pointer
    Uns32 i;
    for(i=0; i<OR1K_TLB_SIZE; i++) {
        or1k->tlb[i].set.nextState    = vmirtGetMRUStateTable(OR1K_TLB_SIZE, i);
        or1k->tlb[i].set.currentState = &or1k->mruState;
    }
}
...
void or1kSetTLBEntry(or1kP or1k, Uns32 tlbIndex, Uns32 va, Uns32 pa) {
    ...
    if(tlbIndex==OR1K_TLB_SIZE) {
        tlbIndex = vmirtGetNthStateIndex(
            OR1K_TLB_SIZE, or1k->mruState, OR1K_TLB_SIZE-1
        );
    } else {
        tlbIndex &= (OR1K_TLB_SIZE-1);
    }

    // clip va and pa to valid values
    va &= ~(OR1K_PAGE_SIZE-1);
    pa &= ~(OR1K_PAGE_SIZE-1);
    ...
    vmirtAliasMemoryVM(
        ...
        &or1k->tlb[tlbIndex].set, MEM_PRIV_RW, True, 0
    );
}
