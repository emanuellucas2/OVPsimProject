...
VMI_RD_PRIV_EXCEPT_FN(or1kRdPrivExceptionCB) {
    ...
    if(MEM_AA_IS_TRUE_ACCESS(attrs)) {
        ...
        if(or1k->addrExEvent) {
            vmirtTriggerViewEvent(or1k->addrExEvent);
        }
    }
}
...
inline static Uns32 getTTCR(or1kP or1k) {
    ...
    Uns32 TTCR = or1k->TTCR;

    if(or1k->timerRunning && !or1k->inSaveRestore) {
        TTCR = TTCR - or1k->TTCRSetCount + getThisICount(or1k);
    }
...
static void setTTCR(or1kP or1k, Uns32 TTCR) {
    ...
    // set raw value of TTCR
    or1k->TTCR = TTCR;

    // update fields dependent on TTCR only if save/restore is not active
    if(!or1k->inSaveRestore) {

        // record count at which TTCR was modified
...
void or1kSetTTMR(or1kP or1k, Uns32 TTMR) {
    ...
    if(!oldIP && newIP && !or1k->artifactAccess) {
        or1k->TTMR_IP = 0;
    }
...
VMI_NET_CHANGE_FN(or1kExternalInterrupt) {
    ...
    or1kP or1k     = (or1kP)processor;
    Uns32 deviceId = (UnsPS)userData;

    if(newValue) {
        if(or1k->logFile) {
            fprintf(or1k->logFile, "Taking interrupt with ID=0x%x\n", deviceId);
        }
        or1k->PICSR |= deviceId;
    } else {
        or1k->PICSR &= ~deviceId;
...
VMI_NET_CHANGE_FN(or1kExternalReset) {
    ...
    or1kP or1k     = (or1kP)processor;
    Bool  oldReset = or1k->resetInput;

    // save new value of reset signal
    or1k->resetInput = (newValue!=0);

    if(!oldReset && or1k->resetInput) {

        // reset signal raised: halt processor
        vmirtHalt(processor);

    } else if(oldReset && !or1k->resetInput) {

        // reset signal lowered: restart processor
