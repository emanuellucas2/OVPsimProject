...
//
static NET_WRITE_FN(exchCountMonitor) {
    icmPrintf(">>> exchCount net written with %u\n", (Uns32)value);
}
...
int main(int argc, const char **argv) {
    ...
    icmNetP exchCountNet = icmNewNet("exchCountNet");

    // connect exchCountNet net to the processor
    icmConnectProcessorNet(processor, exchCountNet, "exchCountNet", ICM_OUTPUT);

    // install callback monitoring net
    icmAddNetCallback(exchCountNet, exchCountMonitor, 0);
