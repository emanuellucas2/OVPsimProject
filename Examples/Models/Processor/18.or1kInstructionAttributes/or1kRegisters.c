...
//
#define OR1K_REG_IMPL_RAW(_REG, _FIELD, _BITS) \
    vmirtRegImplRaw(processor, _REG, _FIELD, _BITS)

//
// Helper macro for defining ignored registers
//
#define OR1K_REG_IMPL_IGNORE(_FIELD, _BITS) \
    OR1K_REG_IMPL_RAW(0, _FIELD, _BITS)
...
#if(ENABLE_ATTRS)
    // specify that flag registers are in SR
    vmiRegInfoCP CPSR = vmirtGetRegByName(processor, "SR");
    OR1K_REG_IMPL_RAW(CPSR, OR1K_CARRY,    8);
    OR1K_REG_IMPL_RAW(CPSR, OR1K_OVERFLOW, 8);
    OR1K_REG_IMPL_RAW(CPSR, OR1K_BRANCH,   8);
#endif
}
